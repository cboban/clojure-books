...
(defn get-user-shelves
  ...
  []
  (for [[id shelve-data book-count] (:data (n4j/cypher-query (str "MATCH(user:User)-[:OWNS]->(shelves)-[:STORES]->(books) WHERE id(user) = " (:id (session-get :user)) " RETURN id(shelves), shelves, count(books)")))]
